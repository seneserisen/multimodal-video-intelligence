...
Media-probe and audio-extraction tests inject deterministic command runners and never execute external binaries. Faster-Whisper tests inject a local model-shaped fake and validate the complete-local-manifest boundary, `local_files_only=True`, paths with spaces, timestamp monotonicity, language/confidence/provenance mapping, lazy iteration failure, redaction, and cooperative cancellation without loading a model or contacting a network. A real `inspect-media` smoke test is environment-dependent and must be reported as blocked when `ffprobe` is absent. Real local ASR validation must be reported as unavailable when the optional package and local model are not configured. Real browser playback, recording, signal-level video/audio validation, OCR, visual analysis, and cloud provider APIs remain outside the implemented scope.

Local real-provider acceptance is separate from automated/CI verification. It must record the authorized fixture, exact local model, device/compute type, durations, segment/language/provenance results, persistence, and backup/restore. `doctor` checks readiness only and never labels inference verified. CI must not install the transcription extra, download a model/tokenizer, or contact Hugging Face.
